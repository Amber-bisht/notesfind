...
export default function WebinarClient({ webinar }: WebinarClientProps) {
    ...
    const checkIfJoined = useCallback((currentWebinar: Webinar, currentUser: any) => { // eslint-disable-line @typescript-eslint/no-explicit-any
        if (!currentUser || !currentWebinar) return;
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const joined = currentUser.joinedWebinars?.some((jw: any) =>
            (jw.webinarId?._id || jw.webinarId) === currentWebinar._id
        );
        setIsJoined(!!joined);
    }, []);
